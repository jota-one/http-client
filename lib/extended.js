...
import { HateoasHttpClient } from './http'
...
export default function ({ axiosConfig, rootEndpoint = '/index', enableLogging = false, disableCache = false }) {
  const hateoas = HateoasHttpClient.create({
  ...
  return {
    ...
    async _getCachedValue(
      ...
      let loadFn = cachePath =>
        this._followLink(resource, key, params, axiosOptions, cachePath)
      if (!resource && !key) {
        loadFn = () => this._getRootIndex()
      }
      if (
        process.env.VUE_APP_CACHE_DISABLED ||
    ...
    async _getRootIndex() {
      ...
      return hateoas.loadIndex(rootEndpoint)
    },
    ...
    async post(
      ...
      return this._followLink(resource, rel, urlPlaceholders, {
        method: 'post',
        data: payload,
        ...axiosOptions,
      })
    ...
    delete(resource, rel, params = {}) {
      return this._followLink(resource, rel, params, {
        method: 'delete',
      })
    }
  }
