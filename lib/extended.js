...
const cloneDeep = obj => mergician({}, obj)
const merge = (obj1, obj2) => mergician(obj1, obj2)
...
export default function extended({
  ...
  const getRootIndex = async () => {
    ...
    return client.loadIndex(rootEndpoint, axiosConfigCb())
      .then(response => {
  ...
  const followLink = async (resource, key, params, axiosOptions, cachePath) => {
    try {
      ...
      let r = await client.followLink(resource, key, params, merge(axiosOptions, axiosConfigCb()))

      for (let proc of responseProcessors) {
        r = await proc(r, merge(axiosOptions, axiosConfigCb()))
      }
