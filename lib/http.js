...
function HateoasAxios(config) {
  ...
  const hateoas = {
    loadIndex: (endpoint) => extended.getEndpoint(endpoint),
    ...
    openBinary: (index, rel, params, axiosConfig = {}, filename = 'document.pdf', forceDownload = false) => extended.openBinary(ge(index, rel, params), axiosConfig, filename, forceDownload),

    downloadBinary: (index, rel, params, axiosConfig, filename = 'document.pdf') => hateoas.openBinary(index, rel, params, axiosConfig, filename, true),
  }
