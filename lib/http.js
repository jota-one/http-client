      ...
      return extended.getEndpoint(url, {
        ...
        .then(response => {
          if (!filename) {
            ...
            if (!filename) {
              throw new Error('No filename has been found neither in the frontend call or in the Content-Disposition header.')
            }
          }
...
function HateoasAxios(config) {
  ...
  const hateoas = {
    ...
    getRelEndpoint: (index, rel, params, axiosConfig, fullResponse) => extended.getEndpoint(ge(index, rel, params), axiosConfig, fullResponse),
